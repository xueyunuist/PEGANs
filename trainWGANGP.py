...
datasets = ['cifar10.32', 'stl10.48', 'cat.128']
...
flags.DEFINE_integer('n_dis', 3, "update Generator every this steps")
...
flags.DEFINE_string('logdir', './logs/WGAN_GP_CNN_CIFAR10', 'log folder')
    ...
    with trange(start, FLAGS.total_steps + 1, ncols=0,
        ...
        for step in pbar:
            ...
            for _ in range(FLAGS.n_dis):
                ...
                loss_gp = cacl_gradient_penalty(net_D, x_real, x_fake)
                if FLAGS.cr > 0:
                    loss_cr = consistency_loss(
                        net_D, x_real, y_real, pred_real)
                else:
                    loss_cr = torch.tensor(0.)
                loss_all = loss + FLAGS.cr * loss_cr + loss_gp * FLAGS.alpha
                loss_all.backward()
                optim_D.step()
